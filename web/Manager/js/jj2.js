...
var jj = function (selector) {
     ...
     */
    this.jjAjax = function (needLoader, servletName) {
        $.ajax({
            ...
            data: this.selector,
            beforeSend: function () {
                if (needLoader == "1" || needLoader == true) {
                    $(".ajaxLoaderDiv").show(); // Load ajax loader
                }
            },
            complete: function () {
             ...
             */
            this.jjAjax22 = function (isCacheable, servletName) {
...
//                var newURL = ;
                $.ajax({
                    ...
                    data: this.selector,
                    beforeSend: function () {
                        $("#loader").css({display: 'block'});//BY EBRAHEMI
                        $(".ajaxLoaderDiv").show(); // Load ajax loader in div by ajaxLoaderDiv class name //============ BY RASHIDI ========
                        if (newURL.indexOf("&panel=sw") > 0) { //can add=> ||newURL.indexOf("&panel=swRight&")
                            stack[window.location] = $('#maincontent').html();
                            var newTitle = newURL.replace(/Server2.*&text=/, '');
                    ...
                    success: function (data) {
                        ...
                        if (newURL.indexOf("&panel=sw") > 0) {//( if "&panel=sw" is not in url it not worked)
                            if (!($("html.ie-ie9").length > 0 || $("html.lt-ie8").length > 0)) {// not work in IE 9,8,7,...
                                ...
                                stack[window.location] = $('#maincontent').html();
                                $(".ajaxLoaderDiv").hide(); // hide ajax loader in div by ajaxLoaderDiv class name //============ BY RASHIDI ========
                                $("#loader").css({display: 'none'});//BY EBRAHEMI
            ...
            this.jjAjax3 = function (isCacheable, servletName) {
                if (sessionStorage.getItem("is_reloaded"))
                    alert('Reloaded!');
//                var newURL = "Server2?" + this.selector;//اگر این خط فعال باشد هر چی سمت سرور ارسال شود در  ادرس دیده میشود
                var newURL = "Server2?";
    ...
    this.jjDigitOnly = function (max, min) {
        $(this.selector).on("focusout", function () {
            var val = $(this).val();
            if (!(new jj(val).jjIsFloat())) {//اگر مقدار عدد نبود
                $(this).val($(this).val().replaceAll(/\D/, ""));
            }
        });
...
//                    alert("333333333333333333333333333333");
                } else if ($(com_textarea[i]).hasClass("ck")) {//writen by s@fe-paradise
//                    alert("11111111111111111111111111111");
                    var idname = com_textarea[i].id;
//                    alert(idname);
                    var val = eval("CKEDITOR.instances." + idname + ".getData()");
//                    alert(val);
                    param += (param == "" ? "" : "&") + com_textarea[i].name + "=" + val;
//                    for (var i = 0; i < com_textarea.length; i++) {
...
//                    alert("2222222222222222222222222");
    ...
    this.jjFormClean = function () {
        $(this.selector + " .imageCleanerUpload").remove();// دکمه هایی که در بخاطر آپلود عکس بوجود آمده اند در   jjAjaxFileUpload2
        var com_checkbox = $(this.selector + " input:checkbox");
        var com_text = $(this.selector + " input:text");
        var com_number = $(this.selector + " input[type='number']");
        var com_text_hiden = +$(this.selector + " input:hidden");
        ...
        for (var i = 0; i < com_text.length; i++) {
            if (com_text[i].name != "") {
                new jj(com_text[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_number.length; i++) {
            if (com_number[i].name != "") {
                new jj(com_number[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_text_hiden.length; i++) {
            if (com_text_hiden[i].name != "") {
                new jj(com_text_hiden[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_checkbox.length; i++) {
            if (com_checkbox[i].name != "") {
                new jj(com_checkbox[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_radio.length; i++) {
            if (com_radio[i].name != "") {
                new jj(com_radio[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_select.length; i++) {
        ...
        for (var i = 0; i < com_password.length; i++) {
            if (com_password[i].name != "") {
                new jj(com_password[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_textarea.length; i++) {
            if (com_textarea[i].name != "") {
                new jj(com_textarea[i]).jjVal('');
            }
        }
        for (var i = 0; i < com_file.length; i++) {
            if (com_file[i].name != "") {
                new jj(com_file[i]).jjVal('');
            }
        }
    };
    /**
...
//         $.cookie(this.selector + "=; expires=Fri, 21 Dec 1976 04:31:24 GMT;"); // BY RASHIDI ===> WITH JQUERY
    ...
    this.jjCookieUpdate = function (value) {
        var x = new jj($.cookie(this.selector)).jjIsDigit() ? parseInt($.cookie(this.selector)) : $.cookie(this.selector);
        value = new jj(value).jjIsDigit() ? parseInt(value) : value;
...
//        var aCookie = document.cookie$.cookie(this.selector).split("; expires")[0]; // BY RASHIDI ===> WITH JQUERY
     ...
     */
    this.jjModalExtraLarge = function (myTitle) {
                                ...
                                if (data != "big") {
                                    ...
                                    for (var j = 0; j < temp.size(); j++) {
                                        if (ext == ("png") || ext == ("jpg") || ext == ("gif") || ext == ("svg")) {
                                            ...
                                            $(divUploadSelector).append("<div><input  class='col-xs-12 form-control ' disabled='disabled' value='" + $("#" + inputFiletitle.replace("#", "")).val() + "'>"
                                                    + " <input class='col-xs-12 form-control file' disabled='disabled' value='" + data + "'>"
                                                    + "<button  class='buttonRemove col-lg-1' style='background-color: #e16262;color: white;float:left' onclick=' $(this).parent().remove();' >حذف</button>"
                                                    + "<a  class='col-lg-1' style='background-color: green;color: white;float:left;text-align: center;padding-top: 2px;padding-bottom: 1px;margin-top: 1px;' href='upload/" + data + "' >دانلود</a></div>");
                                        }
                    ...
                    $.ajaxFileUpload({
                        ...
                        success: function (data) {
                            ...
                            for (var i = 0; i < dataArray.length; i++) {
                                if (dataArray[i] != "") {
                                    if (dataArray[i] != null) {
                                        dataArray[i] = dataArray[i].replace('<pre style="word-wrap: break-word; white-space: pre-wrap;">', '');
                                        dataArray[i] = dataArray[i].replace('<PRE style="word-wrap: break-word; white-space: pre-wrap;">', '');
                                        dataArray[i] = dataArray[i].replace("<PRE>", '').replace("</PRE>", '').replace("<pre>", '').replace("</pre>", '').replace("upload/", '').replace("Upload/", '');
                                        dataArray[i] = dataArray[i].replace("/", '').replace("/", '').replace("\\", '');
                                    } else {
                                        new jj('فایل به درستی ارسال نشد.').jjModal();
                                        return;
                                    }
                                    ...
                                    if (dataArray[i] != "big") {
                                        ...
                                        var param = "";
                                        param += "&title=" + title;
                                        param += "&filename=" + dataArray[i];
                                        param += "&do=Upload.setTitle";
                                        new jj(param).jjAjax2(false);
                                        $("#" + inputFileId.replace("#", "")).val('');
                                        var picName = dataArray[i];
...
//                 
             ...
             * @param {type} inputTextSelector
             * @param {type} inputFiletitle
             * @param {type} viewImgSelector
             * @return {undefined}
             */
                ...
                $(btn).click(function () {
                    if ($("#" + inputFileId.replace("#", "").replace(",", "")).val() == "") {
                        ...
                        $.ajaxFileUpload({
                            ...
                            cache: false,
                            success: function (data) {
                                // برای حذف عکس بارگذاری شده
                                var id = "imageCleanerUpload" + Math.floor(Math.random() * 10000);
                                $(viewImgSelector).before("<span class='imageCleanerUpload' id='" + id + "'><i class='alert-danger icon ion-close'></i></span>");
                                $("#" + id).click(function () {
                                    $("#" + inputTextSelector.replace("#", "")).val("");// پاک کردن تکست ادیتور محتوای اسم عکس
...
//                                                                data = data.replace("/", '').replace("/", '').replace("\\", '');
            ...
            ////////////////
            this.jjAjaxFileUploadOnChange = function (inputFileId, inputTextSelector, inputFiletitle, viewImgSelector) {
                var btn = this.selector;
                $(btn).click(function () {
...
//                                                                data = data.replace("/", '').replace("/", '').replace("\\", '');
     ...
     * 
     @ by safe-paradise
     این متد علاوه بر فرستادن هر نوع فایل اعم از 
     عکس فیلم وفایل های پی دی اف 
     میتواند تایتل و زیرنویس هم را ذخیره کند 
     */
    this.jjAjaxFileUploadByTitleAndMultiFileAndsub = function (inputFileId, databaseColoumnName, inputFiletitle, divUploadSelector, subtitle) {//این متد دارای پنج متغیر ورودی است
        $(this.selector).click(function () {//وقتی روی سلکتور مربوط به این تابع کلیک شد 
            if ($("#" + inputFileId.replace("#", "")).val() == "") {//اگر اون قسمتی که دارای دیس پلی نان هست خالی بود 

                new jj("ابتدا  فایلی را انتخاب نمایید.").jjModal();//یک آلرت بده که ابتدا یک فایل انتخاب کنین
                return;
            }
            $.ajaxFileUpload({
                url: 'UploadServlet',
                ...
                fileElementId: inputFileId.replace("#", ""), //
                fileElementTitle: inputFiletitle.replace("#", ""), //
                fileSubtitle: subtitle.replace("#", ""),
                dataType: 'JSON',
                cache: false,
                success: function (data) {
                    if (data != "") {//اگر دیتا خالی نبود 
                        if (data != null) {//اگر دیتا نال نبود 
                            data = data.replace('<pre style="word-wrap: break-word; white-space: pre-wrap;">', '');//این دیو ها برای پیش نمایش کوچکی از فایل ها بکار میروند 
                            data = data.replace('<PRE style="word-wrap: break-word; white-space: pre-wrap;">', '');
                            data = data.replace("<PRE>", '').replace("</PRE>", '').replace("<pre>", '').replace("</pre>", '').replace("upload/", '').replace("Upload/", '');
                            data = data.replace("/", '').replace("/", '').replace("\\", '');
                        } else {//اگر دیتا خالی بود 
                            new jj('فایل به درستی ارسال نشد.').jjModal();
                            return;
                        }
                        $("#" + inputFileId.replace("#", "")).val('');
                        if (data != "big") {//اگر دیتا بزرگ نبود 
                            alert(data);
                            var title = $("#" + inputFiletitle.replace("#", "")).val();
                            var sub = $("#" + subtitle.replace("#", "")).val();//مقدار داده ی ورودی را از قسمتی که برای زیر نویس برای ان درنظر گرفته شده است را دریافت میکند و انرا در ظرف ساب میگذارد 
                            var param = "";
                            param += "&title=" + title;
                            param += "&filename=" + data;
//                            alert("subtitle" + sub);
                            param += "&subtitle=" + sub;
                            param += "&do=Upload.setTitle";//تابع اپلود دات ست ساب فراخوانی شده و اجرا میگردد
//                            param +="&do=Upload.setSubtitle";
                            new jj(param).jjAjax2(false);//مقادیر به سمت سرور فرستاده میشود 
                            $("#" + inputFileId.replace("#", "")).val('');
                            var picName = data;
                            var ext = data.split('.').pop();
